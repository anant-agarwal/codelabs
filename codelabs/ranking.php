...
-->
	...
	<div  style=" margin:auto">
		...
		<?php
					...
					$exec=mysql_query("SELECT * FROM $contest_table WHERE $cid='$_GET[cid]'",$conn) or die("unable to read Contest name from Contest  table ".mysql_error());						
					$row=mysql_fetch_array($exec);
					$cont_name=$row[$name];
					$exec=mysql_query("SELECT * FROM ranking WHERE $cid='$_GET[cid]' ORDER BY points DESC",$conn) or die("unable to read from Ranking  table ".mysql_error());						
		...
		echo"<h2>Ranking for ".$cont_name." </br>
		</h2>
			<div style='width:20%;float:left;text-align:center' class='status_div1'>
			Rank
			</div>
			<div style='width:60%;float:left;text-align:center' class='status_div1'>
			User Name
			</div>
			<div style='width:20%;float:left;text-align:center' class='status_div1'>
			Points
					...
					while($row=mysql_fetch_array($exec))
					{
						...
						echo"<div style='width:20%;float:left;text-align:center' class='status_div'>
							".$i."
							</div>
							<div style='width:60%;float:left;text-align:center' class='status_div'>
							".$row['user']."
							</div>
					...
					if($i==1)
					echo "<div class='bg'></div><div style='width:100%;float:left;text-align:center' class='status_div'>no one has submitted anything till now!!!</div>";
	 				mysql_close($conn);
				?>
